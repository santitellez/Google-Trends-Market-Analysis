...
condensed_keywords <- c("stock", "crash", "bubble", "loan")
...
search_results <- ggplot(data = results) +
  geom_line(aes(date, smooth(stock, kind = smoother)), color = "blue") +
  geom_line(aes(date, smooth(crash, kind = smoother)), color = "red") +
  geom_line(aes(date, smooth(bubble, kind = smoother)), color = "green") +
  geom_line(aes(date, smooth(loan, kind = smoother)), color = "orange") +
  xlab("Date") +
  ylab("Hits") +
...
lags <- 100
for (lag in 1:lags) {
  for (word in condensed_keywords) {
...
Data <- na.omit(Data)
Data <- Data[, -which(colnames(Data) %in% condensed_keywords)]
